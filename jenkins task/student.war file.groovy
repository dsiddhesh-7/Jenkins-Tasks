pipeline{
    ...
    stages{
        ...
        {
            steps{
                sh ' wget https://dlcdn.apache.org/tomcat/tomcat-8/v8.5.99/bin/apache-tomcat-8.5.99.tar.gz'
                sh ' tar -xvf apache-tomcat-8.5.99.tar.gz'
            }
        }
        stage('Deploying war file')
        {
            steps{
                sh "echo '1234' | mv  webapp/student.war apache-tomcat-8.5.99/webapps"
            }
        }
        stage('Starting Apache 8')
        {
            steps{
                sh './apache-tomcat-8.5.99/bin/catalina.sh start'
            }
        }
    }
}